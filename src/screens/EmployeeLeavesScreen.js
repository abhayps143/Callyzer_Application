        ...
        <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
            <View style={mStyles.container}>
                <View style={mStyles.header}>
                    <View><Text style={mStyles.title}>Apply for Leave</Text><Text style={mStyles.subtitle}>Submit request for HR approval</Text></View>
                    <TouchableOpacity onPress={onClose} style={mStyles.closeBtn}><Text style={mStyles.closeText}>✕</Text></TouchableOpacity>
                </View>
                    ...
                    <View style={mStyles.section}>
                        ...
                        <View style={mStyles.typeGrid}>
                            {leaveTypes.map(type => {
                                ...
                                return (
                                    <TouchableOpacity key={type} style={[mStyles.typeBtn, active && { backgroundColor: cfg.bg, borderColor: cfg.color }]} onPress={() => set('leaveType', type)}>
                                        <Text style={mStyles.typeIcon}>{cfg.icon}</Text>
                                        <Text style={[mStyles.typeLabel, active && { color: cfg.color, fontWeight: '700' }]}>{cfg.label}</Text>
                                    </TouchableOpacity>
                                );
                            })}
                        </View>
                    ...
                    <View style={mStyles.section}>
                        ...
                        <View style={mStyles.dateRow}>
                            <View style={mStyles.dateField}>
                                ...
                                <TextInput style={mStyles.dateInput} value={form.fromDate} onChangeText={v => { set('fromDate', v); if (form.toDate < v) set('toDate', v); }} placeholder="YYYY-MM-DD" placeholderTextColor="#475569" />
                            </View>
                            <View style={mStyles.dateSeparator}><Text style={mStyles.dateSeparatorText}>→</Text></View>
                            <View style={mStyles.dateField}>
                                <Text style={mStyles.dateFieldLabel}>To Date</Text>
                                <TextInput style={mStyles.dateInput} value={form.toDate} onChangeText={v => set('toDate', v)} placeholder="YYYY-MM-DD" placeholderTextColor="#475569" />
                            </View>
                        </View>
                        {form.fromDate && form.toDate && (
                            <View style={mStyles.daysPreview}><Text style={mStyles.daysPreviewText}>📅 <Text style={{ fontWeight: 'bold' }}>{days}</Text> working day{days !== 1 ? 's' : ''} requested</Text></View>
                        )}
                    </View>
                    <View style={mStyles.section}>
                        <Text style={mStyles.label}>Reason (Optional)</Text>
                        <TextInput style={mStyles.textarea} value={form.reason} onChangeText={v => set('reason', v)} placeholder="Briefly describe the reason..." placeholderTextColor="#475569" multiline numberOfLines={4} textAlignVertical="top" />
                    </View>
                    <View style={{ height: 20 }} />
                </ScrollView>
                <View style={mStyles.footer}>
                    <TouchableOpacity style={mStyles.cancelBtn} onPress={onClose}><Text style={mStyles.cancelBtnText}>Cancel</Text></TouchableOpacity>
                    <TouchableOpacity style={[mStyles.submitBtn, saving && { opacity: 0.7 }]} onPress={handleSubmit} disabled={saving}>
                        {saving ? <ActivityIndicator color="#fff" /> : <Text style={mStyles.submitBtnText}>📤 Submit Request</Text>}
                    </TouchableOpacity>
                </View>
            </View>
        </Modal>
    ...
    return (
        <View style={styles.card}>
            <View style={styles.cardTop}>
                <View style={[styles.typeBadge, { backgroundColor: type.bg }]}><Text style={[styles.typeBadgeText, { color: type.color }]}>{type.icon} {type.label}</Text></View>
                <View style={[styles.statusBadge, { backgroundColor: status.bg }]}><Text style={[styles.statusBadgeText, { color: status.color }]}>{status.icon} {status.label}</Text></View>
            </View>
            <View style={styles.cardMid}>
                <View style={styles.dateBlock}><Text style={styles.dateLabel}>From</Text><Text style={styles.dateValue}>{fmt(leave.fromDate)}</Text></View>
                <View style={styles.dateArrow}><Text style={styles.dateArrowText}>→</Text></View>
                <View style={styles.dateBlock}><Text style={styles.dateLabel}>To</Text><Text style={styles.dateValue}>{fmt(leave.toDate)}</Text></View>
                <View style={styles.daysBlock}><Text style={styles.daysValue}>{leave.days || 1}d</Text><Text style={styles.daysLabel}>days</Text></View>
            </View>
            {leave.reason ? <Text style={styles.reason} numberOfLines={2}>{leave.reason}</Text> : null}
            {leave.remarks ? <View style={styles.remarksBox}><Text style={styles.remarksLabel}>HR Remarks:</Text><Text style={styles.remarksText}>"{leave.remarks}"</Text></View> : null}
...
export default function EmployeeLeavesScreen() {
    ...
    return (
        <View style={styles.container}>
            <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#f59e0b" />} showsVerticalScrollIndicator={false}>
                <View style={styles.header}>
                    <View><Text style={styles.title}>🗓️ My Leaves</Text><Text style={styles.subtitle}>Track and apply for leave</Text></View>
                    <TouchableOpacity style={styles.applyBtn} onPress={() => setShowModal(true)}><Text style={styles.applyBtnText}>+ Apply</Text></TouchableOpacity>
                </View>

                {error ? <View style={styles.errorBox}><Text style={styles.errorText}>⚠️ {error}</Text><TouchableOpacity onPress={fetchLeaves}><Text style={styles.retryText}>Retry →</Text></TouchableOpacity></View> : null}
                    ...
                    {BALANCE_ITEMS.map(item => {
                        ...
                        return (
                            <View key={item.key} style={styles.balanceCard}>
                                ...
                                <Text style={[styles.balanceValue, { color: item.color }]}>{remaining}<Text style={styles.balanceTotal}>/{item.total}</Text></Text>
                                <Text style={styles.balanceLabel}>{item.label}</Text>
                                <View style={styles.balanceBarBg}><View style={[styles.balanceBarFill, { width: `${pct}%`, backgroundColor: item.color }]} /></View>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tabsScroll} contentContainerStyle={{ paddingHorizontal: 16 }}>
                    {FILTER_TABS.map(tab => (
                        <TouchableOpacity key={tab.key} style={[styles.tab, filter === tab.key && styles.tabActive]} onPress={() => setFilter(tab.key)}>
                            <Text style={[styles.tabText, filter === tab.key && styles.tabTextActive]}>{tab.label} ({counts[tab.key]})</Text>
                        </TouchableOpacity>
                    ))}
                </ScrollView>
                ...
                <View style={styles.listContainer}>
                    {loading ? (
                        <View style={styles.center}><ActivityIndicator size="large" color="#f59e0b" /><Text style={styles.loadingText}>Loading leaves...</Text></View>
                    ) : displayed.length === 0 ? (
                        <View style={styles.empty}>
                            <Text style={styles.emptyIcon}>📭</Text>
                            <Text style={styles.emptyText}>{filter !== 'all' ? `No ${filter} leaves found` : 'No leave requests yet'}</Text>
                            {filter === 'all' && <TouchableOpacity style={styles.applyFirstBtn} onPress={() => setShowModal(true)}><Text style={styles.applyFirstBtnText}>Apply for your first leave →</Text></TouchableOpacity>}
                        </View>
                    ) : (
                        displayed.map((leave, i) => <LeaveCard key={leave._id || i} leave={leave} />)
                    )}
                </View>
...
const styles = StyleSheet.create({
    ...
    header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 20, paddingTop: Platform.OS === 'ios' ? 50 : 20, backgroundColor: '#1e293b', borderBottomLeftRadius: 20, borderBottomRightRadius: 20, marginBottom: 16 },
    ...
    balanceCard: { flex: 1, backgroundColor: '#1e293b', borderRadius: 14, padding: 14, alignItems: 'center' },
    ...
    tab: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 1, borderColor: '#334155', marginRight: 8 },
...
const mStyles = StyleSheet.create({
    container: { flex: 1, backgroundColor: '#0f172a' },
    header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', padding: 20, paddingTop: Platform.OS === 'ios' ? 50 : 20, backgroundColor: '#1e293b', borderBottomWidth: 1, borderBottomColor: '#334155' },
    title: { color: '#fff', fontSize: 20, fontWeight: 'bold' },
    subtitle: { color: '#64748b', fontSize: 13, marginTop: 3 },
    closeBtn: { padding: 4 }, closeText: { color: '#64748b', fontSize: 20, fontWeight: '600' },
    body: { flex: 1 },
    section: { paddingHorizontal: 16, paddingTop: 20 },
    label: { color: '#94a3b8', fontSize: 13, fontWeight: '600', marginBottom: 10 },
    typeGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
    typeBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, borderWidth: 2, borderColor: '#334155', backgroundColor: '#1e293b', width: '47%' },
    typeIcon: { fontSize: 18 }, typeLabel: { color: '#64748b', fontSize: 13, fontWeight: '500' },
    dateRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
    dateField: { flex: 1 }, dateFieldLabel: { color: '#64748b', fontSize: 12, marginBottom: 6 },
    dateInput: { backgroundColor: '#1e293b', color: '#fff', borderRadius: 10, padding: 12, fontSize: 14, borderWidth: 1, borderColor: '#334155' },
    dateSeparator: { paddingTop: 20 }, dateSeparatorText: { color: '#475569', fontSize: 18 },
    daysPreview: { backgroundColor: '#3b82f615', borderRadius: 10, padding: 12, marginTop: 12, borderWidth: 1, borderColor: '#3b82f630' },
    daysPreviewText: { color: '#3b82f6', fontSize: 14, textAlign: 'center' },
    textarea: { backgroundColor: '#1e293b', color: '#fff', borderRadius: 12, padding: 14, fontSize: 14, borderWidth: 1, borderColor: '#334155', minHeight: 90 },
    errorBox: { margin: 16, backgroundColor: '#ef444415', borderRadius: 10, padding: 12, borderWidth: 1, borderColor: '#ef444440' },
    errorText: { color: '#ef4444', fontSize: 13 },
    footer: { flexDirection: 'row', padding: 16, gap: 10, borderTopWidth: 1, borderTopColor: '#1e293b' },
